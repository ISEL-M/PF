function [bit_lsp, lspq]=code_lsp(lsp)
...
[ind, lspq(1)]=quantiz(lsp(1), vd1, vq1);
bit_lsp=de2bi(ind, 3, 'left-msb');

[ind, lspq(2)]=quantiz(lsp(2), vd2, vq2);
while(lspq(2)<lspq(1))
    ind=ind+1;
    lspq(2)=vq2(ind+1);
end
bit_lsp = [bit_lsp de2bi(ind, 4, 'left-msb')];

[ind, lspq(3)]=quantiz(lsp(3), vd3, vq3);
while(lspq(3)<lspq(2))
    ind=ind+1;
    lspq(3)=vq3(ind+1);
end
...
[ind, lspq(4)]=quantiz(lsp(4), vd4, vq4);
while(lspq(4)<lspq(3))
    ind=ind+1;
    lspq(4)=vq4(ind+1);
end
...
[ind, lspq(5)]=quantiz(lsp(5), vd5, vq5);
while(lspq(5)<lspq(4))
    ind=ind+1;
    lspq(5)=vq5(ind+1);
end
...
[ind, lspq(6)]=quantiz(lsp(6), vd6, vq6);
while(lspq(6)<lspq(5))
    ind=ind+1;
    lspq(6)=vq6(ind+1);
end
bit_lsp = [bit_lsp de2bi(ind, 3, 'left-msb')];

[ind, lspq(7)]=quantiz(lsp(7), vd7, vq7);
while(lspq(7)<lspq(6))
    ind=ind+1;
    lspq(7)=vq7(ind+1);
end
...
[ind, lspq(8)]=quantiz(lsp(8), vd8, vq8);
while(lspq(8)<lspq(7))
    ind=ind+1;
    lspq(8)=vq8(ind+1);
end
...
[ind, lspq(9)]=quantiz(lsp(9), vd9, vq9);
while(lspq(9)<lspq(8))
    ind=ind+1;
    lspq(9)=vq9(ind+1);
end
...
[ind, lspq(10)]=quantiz(lsp(10), vd10, vq10);
while(lspq(10)<lspq(9))
    ind=ind+1;
    lspq(10)=vq10(ind+1);
end
bit_lsp = [bit_lsp de2bi(ind, 3, 'left-msb')];